...
\def\VERSIONE{5.0}
\def\DATA{29.05.2016}
\def\REDATTORE{Vassilikì Menarin}
...
\myparagraph{Consegna al committente}
La consegna avverrà per mezzo di CD-ROM / DVD, comprensivo di:
\begin{itemize}
	\item manuale utente e manutentore contenenti istruzioni per l’installazione e l’uso;
	\item utilità di installazione;
	\item sorgenti completi;
	\item documentazione completa e finale;
	\item eventuali utilità di collaudo.
\end{itemize}

\myparagraph{Consegna al proponente}
...
Questo documento verrà redatto dai \italics{Progettisti} e ha il compito di descrivere l'architettura ad alto livello del prodotto identificando le componenti al proprio interno.
Prima di procedere alla definizione vera e propria dell’architettura del prodotto software,
devono essere eseguiti i seguenti task:
\begin{itemize}
	\item si suddivide il prodotto in componenti;
	\item si definiscono ruoli e responsabilità di ognuno dei componenti individuati;
	\item si definiscono le interazioni che i componenti hanno fra di loro;
	\item si definiscono le eventuali interfacce che ogni componente mette a disposizione dell’ambiente
	esterno;
	\item ci si assicura che ogni requisito sia soddisfatto da almeno uno dei componenti individuati;
	\item ci si assicura che ogni componente realizzi almeno un requisito;
	\item si realizzano e si documentano i test di integrazione, necessari per verificare il corretto funzionamento di più componenti assemblati insieme.
\end{itemize}
Seguono i principali contenuti del documento:
\begin{itemize}
	\item Diagrammi \gl{UML}
	...
	\end{itemize}
	\item Test
	\\ Saranno definite tre tipologie di test:
	\begin{itemize}
		\item test di integrazione,
		\item test di sistema,
		\item test di validazione,
	\end{itemize}
	che permetteranno di verificare il corretto funzionamento dell'intera architettura. In particolare i test di integrazione avranno il compito di individuare i problemi che si potranno verificare nella combinazione di due o più distinte unità.
	\newline Per maggiori dettagli sui test si veda la sezione dedicata in questo documento.
	\item \gl{Design pattern}
	...
	\item Tracciamento componenti
	\\ Infine sarà necessario eseguire il tracciamento requisito-componente atto a controllare lo stato di avanzamento del progetto. Questo tracciamento viene automatizzato dal tool \gl{Tracker}.
\end{itemize}

\mysubparagraph{Definizione di Prodotto}
I \italics{Progettisti} devono redigere la \doc{Definizione di Prodotto}. In essa viene descritta la progettazione di dettaglio del sistema, specificando maggiormente quanto scritto nella \doc{Specifica Tecnica}.
Essa prevede che vengano eseguiti i seguenti task:
\begin{itemize}
	\item si individuano le classi che implementano ciascuno dei componenti trovati durante la progettazione
	architetturale;
	\item si definiscono i ruoli e le responsabilità di ciascuna delle classi individuate;
	\item si definiscono le interazioni che le classi hanno tra di loro;
	\item ognuna delle classi deve essere definita nel dettaglio;
	\item ci si assicura che ogni requisito sia soddisfatto da almeno una delle classi individuate e
	descritte;
	\item ci si assicura che ogni classe realizzi almeno un requisito;
	\item si realizzano e si documentano i test di unità, necessari per verificare il corretto comportamento
...
\begin{itemize}
	...
	\end{itemize}
	I diagrammi di sequenza permetteranno di comprendere come un determinato comportamento del prodotto si svolga in funzione dell'interazione tra diversi oggetti che collettivamente lo implementano.
	\item Test
	\\ Sarà definita la seguente tipologia di test:
	\begin{itemize}
		\item test di unità,
	\end{itemize}
	questi permetteranno di verificare che le singole componenti del sistema funzionino come previsto.
	\item Definizione di classe
	...
	\\ Ad ogni classe deve essere associato almeno un requisito in modo tale da poter controllare lo stato di avanzamento nello sviluppo del progetto secondo il modello specificato nel \doc{Piano di Progetto}. L'applicativo \gl{Tracker} viene utilizzato nel tracciamento di tali dipendenze.
\end{itemize}
...
\newline L'implementazione concreta dell'architettura porta lo sviluppatore a dover installare software di terze parti per procedere con la stesura del codice, a realizzare un prodotto che dovrà esser compreso dagli utenti che ne usufruiranno e che dovrà esser mantenuto attivo dopo il proprio rilascio. Proprio per questo durante l'attività di codifica verranno scritti i documenti \doc{Manuale Utente} e \doc{Manuale Sviluppatore}.
\newline
L'attività di codifica comprende infine l'implementazione dei test che sono stati definiti nelle attività precedenti, in modo tale che, durante l'attività di validazione, sia possibile eseguirli per verificare la correttezza del software prodotto o per rilevarne dei bug.
...
Redatto dai \italics{Programmatori}, il \doc{Manuale Sviluppatore} ha come destinatari gli incaricati a installare e configurare il prodotto, prima del suo reale utilizzo. Questo documento permetterà inoltre di favorire la manutenibilità del prodotto una volta rilasciato sul mercato, l'eventuale estensione di nuove funzionalità (nuove tipologie di domande) e la gestione delle segnalazioni degli utenti finali.
...
L'identificazione dei \gl{package}, individuati all'interno dell'architettura durante il periodo di progettazione, dovrà essere univoca.
...
\item \bold{\gl{Package}} rappresenta il nome di un \gl{package} padre che racchiuderà all'interno altri \gl{package} oppure classi;
\item \bold{SottoPackage} rappresenta il figlio del \gl{package} padre. Non è detto che ogni \gl{package} padre contenga un sottopackage, pertanto è un campo opzionale. Al contrario un sottopackage potrebbe a sua volta contenere altri \gl{package} come figli (sotto-sottopackage) e così via;
\end{itemize} 
Per ogni \gl{package} vengono inoltre definite:
\begin{itemize}
\item una breve descrizione della componente in questione;
\item \bold{componenti contenute:} un elenco delle altre componenti contenute all'interno del proprio \gl{package};
\item \bold{interazioni on altre componenti:} le interazioni del \gl{package} con le altre componenti, interne oppure uscenti;
\end{itemize}
...
dove:
\begin{itemize}
\item \bold{Quizzipedia, \gl{Package}, SottoPackage} sono descritti nella sezione precedente;
\item \bold{Classe} rappresenta la componente più piccola, contenuta nel \gl{package}, che contiene attributi e metodi.
\end{itemize}
...
\label{sez:test}
Segue un elenco dei tipi di test che vengono effettuati, con la descrizione del modo in cui vengono definiti all'interno del \doc{Piano di Qualifica}.
...
Il codice identificativo dei test di unità è: \centra{TU[codice numerico]} dove il codice numerico è progressivo e determinato dallo strumento \gl{Tracker} in cui vengono inseriti i test di unità.
...
\begin{itemize}
	\item nome del file comprensivo di estensione e percorso a partire dalla root del \gl{repository};
	\item autore e data di creazione del file;
	\item e-mail del \gl{team} Vault-Tech;
...
\\
\newline Nell'attività di progettazione lo strumento \gl{Tracker} è stato incrementato con nuove funzionalità per la stesura automatica delle componenti dell'architettura  all'interno dei documenti \doc{Specifica Tecnica} e \doc{Definizione di Prodotto}. 
Anche i test, una volta inseriti nel \gl{Tracker}, verranno automaticamente scritti all'interno dell'appropriato documento.
...
L'\gl{IDE} JetBrains WebStorm 2016.1.1 è utilizzato dal \gl{team} per scrivere il codice.
Webstorm è stato scelto dal \gl{team} per le funzionalità e i vantaggi che esso offre:
\begin{itemize}
	...
	\item JSHint, un tool che aiuta a rilevare errori e potenziali problemi nel codice \gl{JavaScript};
	\item diversi plugin già implementati per lavorare con \gl{Node.js} e \gl{AngularJS};
...
\myparagraph{Gestione modifiche} 
Qualora durante l'attività di verifica si presenti un problema nella documentazione o nel codice sorgente che necessita di una correzione dovrà esser inoltrata una richiesta al \italics{Responsabile} in modo tale che possa constatare l'effettiva esistenza del problema o meno. Se la segnalazione risulta corretta allora sarà compito del \italics{Responsabile} assegnarne a chi ritiene più opportuno la sua modifica. Spetta poi al \italics{Verificatore} constatare che la modifica sia stata effettuata in modo corretto. 
\newline
La richiesta da presentare al \italics{Responsabile} dovrà contenere:
\begin{itemize}
\item autore: il nome e cognome di colui che invia la richiesta di modifica;
\item documento/file: viene indicato quale documento/file presenta un errore da correggere
\item motivazione: spiegazione del perchè è necessario apportare una modifica.
\end{itemize}

Dopo la visione del \italics{Responsabile} sarà necessario aggiungere se la richiesta è stata approvata oppure rifiutata e il membro del team a cui è stato affidata la correzione, in modo da tener traccia dell'intero processo di modifica.

\subsubsection{Strumenti}

\myparagraph{JsMeter}
JsMeter viene utilizzato per analizzare il codice JavaScript. 
Questo strumento permetterà di capire se alcune metriche descritte nel \doc{Piano di Qualifica} sono state rispettate o meno.
\newline È possibile usufruire di tale mezzo attraverso il sito web \url{http://jsmeter.info/}.
\\ Le metriche prese in analisi dallo strumento sono:
\begin{itemize}
...
\myparagraph{JSHint}
JSHint è uno strumento di analisi statica del codice utilizzato per verificare se il codice sorgente JavaScript è conforme alle metriche definite nel documento \doc{Piano di Progetto}.
\newline È possibile usufruire facilmente di tale servizio attraverso il sito web \url{http://jshint.com/}.
\\ Le metriche prese in analisi dallo strumento sono:
\begin{itemize}
\item numero di statement;
\item numero di parametri formali all'interno di un metodo;
\item complessità ciclomatica.
\end{itemize}

\myparagraph{W3C Css Validation Service}
È un servizio online reperibile al sito \url{https://jigsaw.w3.org/css-validator/} che permette di verificare la correttezza della sintassi con cui sono stati scritti i file css.

\myparagraph{W3C Markup Validation Service}
È un servizio online reperibile al sito \url{https://validator.w3.org/} che permette di verificare la correttezza del codice scritto per le pagine html. 

\newpage
\subsection{Processo di validazione}
Il processo di validazione serve per determinare se il prodotto finale è conforme a quanto è stato pianificato. Tale processo verrà effettuato dopo molteplici verifiche effettuate sul prodotto dal momento che, essendo un atto conclusivo, non è desiderabile ottenere un risultato negativo.
\\Si procederà dapprima ad una validazione interna da parte dei componenti del team e poi sarà il proponente a rivalidare il prodotto, per garantire l’indipendenza del processo.
\\ L’attività di validazione consiste nel condurre i test per poi analizzare i risultati e assicurarsi che
il prodotto rispecchi le indicazioni per cui è stato sviluppato.
Per le norme sulla sintassi dei test si rimanda alla sezione \hyperref[sez:test]{Definizione dei test} del presente documento.

\subsubsection{Attività}

\myparagraph{Analisi dinamica}
Il \italics{Verificatore} riesegue tutti i test prestando molta attenzione ai risultati, specialmente a quelli ottenuti dai test di validazione. 
\\Sarà compito del \italics{Responsabile} poi analizzare i risultati ottenuti per decidere se rieseguire nuovamente i test ulteriormente.
\newline In questo modo avviene il collaudo conclusivo del prodotto prima della sua consegna al committente, verificando che soddisfi tutti gli obbiettivi definiti inizialmente nel \doc{Piano di Qualifica}.

\subsubsection{Procedure}

\myparagraph{Esecuzione dei test}
Per l'esecuzione dei test di unità, di integrazione e i test sul database si assume che il \italics{Verificatore} abbia già installato nella propria macchina il package manager npm di node.js.
\\
\\Per eseguire i test riguardanti unicamente il database serve eseguire i seguenti passi:

\begin{itemize}
\item da terminale ci si sposti nella cartella Quizzipedia;
\item in caso non siano presenti, si installino il test framework mocha e la libreria di asserzioni Should.js, tramite il comando \italics{"npm install"};
\item avviare il database MongoDb digitando il comando \italics{"mongod"} da terminale;
\item ci si sposti poi nella cartella Quizzipedia/tests in cui sono presenti i file dei test e si digiti a riga di comando \italics{"npm run testDb"};
\item in caso i test diano risultato positivo, comparirà l’elenco dei metodi testati con il segno di spunta;
\item in caso alcuni test falliscano verrà presentato il messaggio di errore a terminale con segnalato il metodo che è fallito.
\begin{figure}[htp]
\centering
\includegraphics[scale=0.45]{Img/test1.png}
\caption{Superamento test database}
\label{}
\end{figure}

\begin{figure}[htp]
\centering
\includegraphics[scale=0.35]{Img/test2.png}
\caption{Fallimento test database}
\label{}
\end{figure}
...
\newpage
Per eseguire i test lato client e server invece i passaggi che il \italics{Verificatore} dovrà eseguire sono i seguenti:
\begin{itemize}
\item da terminale ci si sposti nella cartella Quizzipedia;
\item in caso non siano presenti, si installino il test framework Karma e il framework Jasmine, tramite il comando \italics{"npm install -g Karma"} seguito da \italics{"npm install"};
\item ci si sposti poi nella cartella Quizzipedia/tests in cui sono presenti i file dei test e si digiti a riga di comando \italics{"npm run testKarma"};
\item durante l’esecuzione del comando si aprirà una finestra del browser che è possibile ignorare e alla fine della procedura compariranno i risultati dei test nel terminale;
\item in caso i test diano risultato positivo, comparirà il numero di test superati con successo;
\item in caso alcuni test falliscano verrà presentato il numero di test superati con successo e il numero di test falliti con relativo errore.
\begin{figure}[htp]
\centering
\includegraphics[scale=0.50]{Img/test3.png}
\caption{Superato test Karma}
\label{}
\end{figure}

\begin{figure}[htp]
\centering
\includegraphics[scale=0.50]{Img/test4.png}
\caption{Fallito test Karma}
\label{}
\end{figure}
...
\subsubsection{Strumenti}
Di seguito sono elencati i vari tool da utilizzare per l'esecuzione dei test.

\myparagraph{Karma}
Lo strumento Karma è utilizzato per automatizzare i test di unità ed integrazione lato client scritti
con il framework Jasmine. Permette di testare il codice su diversi browser, tra cui Chrome.

\myparagraph{Jasmine}
Un framework JavaScript per l’esecuzione di test su codice che utilizza AngularJS.

\myparagraph{Mocha}
Lo strumento Mocha è utilizzato per eseguire i test asincroni lato server asseriti con la libreria Should.js.

\myparagraph{Should.js}
Should.js è una libreria asserzionale, è necessario Mocha per utilizzarla.
