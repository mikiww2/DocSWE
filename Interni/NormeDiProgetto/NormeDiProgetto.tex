...
I \italics{Progettisti} devono redigere la \doc{Definizione di Prodotto}. In essa viene descritta la progettazione di dettaglio del sistema, specificando quanto scritto nella \doc{Specifica Tecnica}. Inoltre opportuni test di unità devono essere definiti per verificare che i componenti del sistema funzionino come previsto.
I seguenti diagrammi UML devono essere realizzati:
\begin{itemize}
	\item diagrammi delle classi,
	\item diagrammi di sequenza.
\end{itemize}
Ogni classe progettata nella \doc{Specifica Tecnica} deve essere descritta all'interno della \doc{Definizione di Prodotto}. Devono essere illustrati lo scopo della classe e la funzionalità da essa modellata. Nella descrizione devono essere inoltre elencati metodi e attributi della classe. Ad ogni classe deve essere associato almeno un requisito in modo tale da poter controllare lo stato di avanzamento nello sviluppo del progetto secondo il modello specificato nel \doc{Piano di Progetto}. L'applicativo Tracker è utilizzato nel tracciamento di tali dipendenze.

\myparagraph{Codifica}
\bold{DA FARE}

\mysubparagraph{Manuale utente}
I \italics{Programmatori} devono redigere il \doc{Manuale Utente}. In esso vengono fornite all'utente del prodotto le informazioni principali per aiutarlo nell'utilizzo e fornirgli una rapida spiegazione delle funzionalità dell'applicazione. Deve essere chiaro, diretto e pienamente comprensibile alla base di utenza a cui il prodotto è destinato.

\mysubparagraph{Manuale sviluppatore}
Redatto dai \italics{Programmatori}, il \doc{Manuale Sviluppatore} ha come destinatari gli incaricati a installare e configurare il prodotto.
...
\item \bold{DA CONTINUARE A SECONDA DI QUELLO CHE VA SCRITTO NELLA DEF DI PROD}
\end{itemize}
...
\begin{itemize}
	...
	\item diversi plugin già implementati per lavorare con Node e Angular;
