...
\def\VERSIONE{4.0}
\def\DATA{26.04.2016}
\def\REDATTORE{Rudy Berton \\ & Michela De Bortoli \\ & Filippo Tesser}
...
\begin{itemize}
	\item manuale utente e sviluppatore contenenti istruzioni per l’installazione e l’uso;
...
\begin{itemize}
	...
	esterno;
	\item ci si assicura che ogni requisito sia soddisfatto da almeno uno dei componenti individuati e
...
\begin{itemize}
	...
	\end{itemize}
	che permetteranno di verificare il corretto funzionamento dell'intera architettura. In particolare i test di integrazione avranno il compito di individuare i problemi che si potranno verificare nella combinazione di due distinte unità.
...
Seguono i principali contenuti del documento:
\begin{itemize}
	\item Diagrammi \gl{UML}.\\ Sarà necessario realizzare i seguenti diagrammi:
	\begin{itemize}
		\item diagrammi delle classi,
	...
	\item Test.\\ Sarà definita la seguente tipologia di test:
	\begin{itemize}
		\item test di unità,
	\end{itemize}
...
\myparagraph{Gestione modifiche} %DA EXT

\subsubsection{Strumenti}
Di seguito sono elencati i vari tool da utilizzare per i test.
...
\myparagraph{Jasmine}
Un frameworkG JavaScriptG per l’esecuzione di test su codice che utilizza AngularJS.
...
Il processo di validazione serve per determinare se il prodotto finale è conforme a quanto è stato
pianificato. Si procederà dapprima ad una validazione interna da parte dei componenti del team e
poi sarà il proponente a rivalidare il prodotto, per garantire l’indipendenza del processo.
L’attività di validazione consiste nel condurre i test per poi analizzare i risultati e assicurarsi che
il prodotto rispecchi le indicazioni per cui è stato sviluppato. Si andrà inoltre a verificare che il
prodotto sia abile nell’isolare e minimizzare gli effetti degli errori.
\subsubsection{Attività}
