...
\newline \\gl{url}{http://www.math.unipd.it/~tullio/IS-1/2015/Progetto/C5.\gl{pdf}};
...
\item \bold{Qualità del \gl{software} - Ercole F. Colonese:}
	\newline \\gl{url}{http://www.colonese.it/00-Manuali_Pubblicatii/06-Qualit%C3%A0Software_v2.\gl{pdf}};
\item \bold{La qualità del \gl{software} secondo il lo ISO/IEC9126 - Ercole F. Colonese:} 
	\newline \\gl{url}{http://www.colonese.it/00-Manuali_Pubblicatii/07-ISO-IEC9126_v2.\gl{pdf}};
\item \bold{Metriche del \gl{software} G - Ercole F. Colonese:}
	\newline \\gl{url}{http://www.colonese.it/00-Manuali_Pubblicatii/08-Metriche%20del%20software_v1.0.\gl{pdf}};
\item \bold{The Guide to the \gl{Software} Engineering Body of Knowledge (SWEBOK Guide v3.0) - IEEE Computer Society:} Chapter 10 - \gl{Software} Quality
	\newline \\gl{url}{http://www.computer.org/\gl{web}/swebok/;jsessionid=e21ab6073d3593a8523de74bddd2};
\item \bold{\gl{PDCA}:}
	\newline \\gl{url}{https://en.\gl{wikipedia}.org/wiki/\gl{PDCA}};
\item \bold{Capability Maturity :}
	\newline \\gl{url}{https://en.\gl{wikipedia}.org/wiki/Capability_Maturity_};
\item \bold{ISO 9126:}
	\newline \\gl{url}{https://en.\gl{wikipedia}.org/wiki/ISO/IEC_9126};
\item \bold{\gl{Indice Gulpease}:}
	\newline \\gl{url}{https://it.\gl{wikipedia}.org/wiki/Indice_Gulpease};
\item \bold{Slide del corso Ingegneria del \gl{Software} - Misurazioni del \gl{Software}:}
	\newline \\gl{url}{http://www.math.unipd.it/~rcardin/sweb/B02.\gl{pdf}};
\item \bold{Complessità ciclomatica:}
	\newline \\gl{url}{https://it.\gl{wikipedia}.org/wiki/Complessit%C3%A0_ciclomatica}.2
\end{itemize}
...
\end{itemize}
Per maggiori dettagli sulla metrica si veda la sezione \hyperref[par:\gl{web}]{Validazione \gl{web}} in appendice.
...
\end{itemize}
Per maggiori dettagli sulla metrica si vedano le sezioni \hyperref[par:\gl{web}]{Validazione \gl{web}} e \hyperref[par:greff]{Grado di efficienza} in appendice.
...
\begin{tabella}{!{\VRule}c!{\VRule}p{8cm}!{\VRule}c!{\VRule}c!{\VRule}}
	...
	\\
	TV10 &
		L’utente vuole verificare il corretto funzionamento del sistema con l’uso di \gl{JavaScript} e \gl{cookies}.
		\newline \newline
		Operazioni:
		{\begin{enumerate}
...
\begin{tabella}{!{\VRule}c!{\VRule}p{8cm}!{\VRule}c!{\VRule}c!{\VRule}}
	...
	\\
	TS10 & Viene verificato che il sistema funzioni correttamente con \gl{JavaScript} e  \gl{cookies} abilitati. & ROV10 & N.I.
	\\
	TS27 & Viene verificato che il sistema funzioni correttamente sul \gl{browser} \gl{Google Chrome} versione 49 e future. & ROV27 & N.I.
	\\
	TS28 & Viene verificato che il sistema funzioni correttamente sul \gl{browser} \gl{Mozilla Firefox} versione 44. & ROV28 & N.I.
	\\
	TS29 & Viene verificato che il sistema funzioni correttamente sul \gl{browser} Internet Explorer versione 11. & ROV29 & N.I.
	\\
	TS30 & Viene verificato che il sistema funzioni correttamente sul \gl{browser} \gl{Safari} versione 9. & ROV30 & N.I.
	\\
	\rowcolor{white}  
...
\begin{tabella}{!{\VRule}l!{\VRule}p{3cm}!{\VRule}p{6cm}!{\VRule}c!{\VRule}}
	...
	tra \gl{client}, \gl{server} e \gl{database} funzionino a dovere e
	...
	front end del prodotto tra le tre componenti Model, Controller e View. & Quizzipedia::\gl{Client} & N.I.
	\\
	TIClient & Viene verificato che il sistema gestisca correttamente la componente Model relativa al \gl{design pattern} \gl{MVC}. Più precisamente, viene verificato che la memorizzazione della struttura dei dati avvenga correttamente. & Quizzipedia::\gl{Client}::\gl{Client} & N.I.
	\\
	TIOrganization & Viene verificato che il sistema gestisca correttamente la memorizzazione della struttura dei dati associati agli enti e alle classi. & Quizzipedia::\gl{Client}::\gl{Client}::\-Organization & N.I.
	\\
	TIRequests & Viene verificato che il sistema gestisca a dovere la memorizzazione della struttura delle diverse richieste in arrivo dal controller. & Quizzipedia::\gl{Client}::\gl{Client}::\-Requests & N.I.
	\\
	TIServicesClient & Viene verificato che il sistema gestisca in modo corretto la memorizzazione della struttura dei dati per i diversi servizi offerti dal prodotto (domande, quiz, argomenti). & Quizzipedia::\gl{Client}::\gl{Client}::\-Services & N.I.
	\\
	TIQuestionsClient & Viene verificato che il sistema gestisca in modo corretto la memorizzazione della struttura dei dati relativi alle diverse tipologie di domande che possono comparire all’interno del sistema. & Quizzipedia::\gl{Client}::\gl{Client}::\-Services::Questions & N.I.
	\\
	TIStatistics & Viene verificato che il sistema gestisca in modo corretto la memorizzazione della struttura dei dati relativi alle statistiche. & Quizzipedia::\gl{Client}::\gl{Client}::\-Statistics & N.I.
	\\
	TIUsers & Viene verificato che il sistema gestisca correttamente la
	memorizzazione della struttura dei dati associati ai diversi utenti. & Quizzipedia::\gl{Client}::\gl{Client}::\-Users & N.I.
	\\
	TIViewClient & Viene verificato che il sistema gestisca correttamente la
	componente View relativa al pattern \gl{MVC}. Più precisamente, viene verificato che venga gestita correttamente la rappresentazione del prodotto al cliente. & Quizzipedia::\gl{Client}::ViewClient & N.I.
	\\
	TIViewOrgManager & Viene verificato che il sistema gestisca in modo corretto le pagine dedicate alla gestione delle classi e degli enti. & Quizzipedia::\gl{Client}::ViewClient::\-ViewOrgManager & N.I.
	\\
	TIViewQuestionManager & Viene verificato che il sistema gestisca correttamente le pagine dedicate alla gestione delle domande. & Quizzipedia::\gl{Client}::ViewClient::\-ViewQuestionManager & N.I.
	\\
	TIViewQuizManager & Viene verificato che il sistema gestisca correttamente le pagine dedicate alla gestione dei quiz. & Quizzipedia::\gl{Client}::ViewClient::\-ViewQuizManager & N.I.
	\\
	TIViewQuizSolver & Viene verificato che il sistema gestisca correttamente le pagine dedicate allo svolgimento dei quiz e alla visualizzazione dei loro risultati. & Quizzipedia::\gl{Client}::ViewClient::\-ViewQuizSolver & N.I.
	\\
	TIViewQuestionSolver & Viene verificato che il sistema gestisca in modo appropriato le pagine dedicate allo svolgimento delle singole domande, a seconda della loro tipologia. & Quizzipedia::\gl{Client}::ViewClient::\-ViewQuizSolver::ViewQuestionSolver & N.I.
	\\
	TIViewRequest & Viene verificato che il sistema gestisca correttamente le pagine rappresentanti la gestione delle richieste di ruolo e di classe. & Quizzipedia::\gl{Client}::ViewClient::\-ViewRequests & N.I.
	\\
	TIViewSearch & Viene verificato che il sistema gestisca in modo corretto le pagine per l’esecuzione della ricerca di classi, quiz e domande. & Quizzipedia::\gl{Client}::ViewClient::\-ViewSearch & N.I.
	\\
	TIViewStatistics & Viene verificata la corretta gestione da parte del sistema delle pagine per la visualizzazione delle statistiche. & Quizzipedia::\gl{Client}::ViewClient::\-ViewStatistics & N.I.
	\\
	TIViewTopicManager & Viene verificato che il sistema gestisca adeguatamente le pagine dedicate alla gestione degli argomenti. & Quizzipedia::\gl{Client}::ViewClient::\-ViewTopicManager & N.I.
	\\
	TIViewUsers & Viene verificato che il sistema gestisca in modo corretto le pagine riguardanti le informazioni di ogni utente e le loro le funzioni principali. & Quizzipedia::\gl{Client}::ViewClient::\-ViewUsers & N.I.
	\\
	TIViewErrors & Viene verificato che il sistema gestisca correttamente la visualizzazione degli errori ricevuti. & Quizzipedia::\gl{Client}::ViewClient::\-ViewErrors & N.I.
	\\
	TIControllerClient & Viene verificato che il sistema gestisca correttamente la componente Controller relativa al \gl{design pattern} \gl{MVC} . Più precisamente, viene verificato che il
	sistema gestisca le azioni dell’utente, collegando in modo adeguato View e Model. & Quizzipedia::\gl{Client}::ControllerClient & N.I.
	\\
	TICtrlOrganization & Viene verificato che il sistema gestisca ottimamente la comunicazione tra la componente della View relativa alla creazione delle classi e degli enti e la componente Model a cui vengono inviate le informazioni da immagazzinare nel \gl{database}.  & Quizzipedia::\gl{Client}::ControllerClient::\-CtrlOrganization & N.I.
	\\
	TICtrlRequests & Viene verificato che il sistema gestisca correttamente la comunicazione tra View e Model per la gestione delle richieste eseguite dall’utente. & Quizzipedia::\gl{Client}::ControllerClient::\-CtrlRequests & N.I.
	\\
	TICtrlServices & Viene verificato che il sistema gestisca correttamente la comunicazione tra View e Model del \gl{client} per la creazione, lo svolgimento e la ricerca di quiz e domande. Deve funzionare correttamente anche la comunicazione con il \gl{database} attraverso il \gl{server} per il salvataggio/caricamento delle informazioni. & Quizzipedia::\gl{Client}::ControllerClient::\-CtrlServices & N.I.
	\\
	TICtrlStatistics & Viene verificato che sia gestita correttamente dal sistema la comunicazione tra Model e View per la gestione delle statistiche richieste dall’utente. & Quizzipedia::\gl{Client}::ControllerClient::\-CtrlStatistics & N.I.
	\\
	TICtrlUsers & Viene verificato che sia gestita correttamente dal sistema la comunicazione tra la View, il Model e il \gl{server} per quanto riguarda le informazioni dell’utente e la loro gestione. & Quizzipedia::\gl{Client}::ControllerClient::\-CtrlUsers & N.I.
	\\
	TIServer & Viene verificato che il sistema
	gestisca correttamente l’interazione tra le componenti del \gl{back-end}. & Quizzipedia::\gl{Server} & N.I.
	\\
	TIControllerServer & Viene verificato che il sistema gestisca in modo corretto le richieste eseguite lato \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer & N.I.
	\\
	TIAuthenticationManager & Viene verificato che il sistema gestisca correttamente i servizi di autenticazioni richiesti dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-AuthenticationManager & N.I.
	\\
	TIClassManager & Viene verificato che il sistema gestisca correttamente le richieste di gestione delle classi provenienti dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-ClassManager & N.I.
	\\
	TICompanyManager & Viene verificato che il sistema gestisca appropriatamente le richieste di gestione degli enti ricevute dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-CompanyManager & N.I.
	\\
	TIProfileManager & Viene verificato che il sistema gestisca appropriatamente le richieste del \gl{client} per la gestione delle informazioni riguardanti i profili degli utenti autenticati. & Quizzipedia::\gl{Server}::ControllerServer::\-ProfileManager & N.I.
	\\
	TIQuestionsManager & Viene verificato che il sistema gestisca in modo corretto la gestione delle domande secondo le richieste ricevute dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-QuestionsManager & N.I.
	\\
	TIQuizManager & Viene verificato che il sistema gestisca in modo corretto la gestione dei quiz secondo le richieste ricevute dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-QuizManager & N.I.
	\\
	TIQMLAgent & gestisca in modo corretto la traduzione delle informazioni inviate dal \gl{client}, da linguaggio \gl{QML} ad un formato corretto per il sistema. Deve essere verificato che avvenga anche il contrario. & Quizzipedia::\gl{Server}::ControllerServer::\-QuizManager::QMLAgent & N.I.
	\\
	TIRequestsManager & Viene verificato che il sistema gestisca in modo appropriato le richieste degli utenti (di ruolo o di entrata in una classe) inviate dal \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-RequestsManager & N.I.
	\\
	TISearchManager & Viene verificato che il sistema gestisca correttamente le informazioni inviate dal \gl{client} qualora un utente abbia eseguito una ricerca lato \gl{client}. Deve essere verificata anche la comunicazione della risposta al \gl{client}. &  Quizzipedia::\gl{Server}::ControllerServer::\-SearchManager & N.I.
	\\
	TIStatisticsManager & Viene verificato che il sistema gestisca correttamente le richieste delle statistiche ricevute dal \gl{client}, riguardanti quiz, domande e utenti. &  Quizzipedia::\gl{Server}::ControllerServer::\-StatisticsManager & N.I.
	\\
	TITopicManager & Viene verificato che il sistema gestisca correttamente le richieste di creazione e di eliminazione di un argomento da parte del \gl{client}. & Quizzipedia::\gl{Server}::ControllerServer::\-TopicManager & N.I.
	\\
	TIServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} tutti i dati ottenuti dal \gl{client}.  &  Quizzipedia::\gl{Server}::\gl{Server} & N.I.
	\\
	TIServicesServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} tutti i dati riguardanti le domande e i quiz. &  Quizzipedia::\gl{Server}::\gl{Server}::\-Services & N.I.
	\\
	TIQuestionsServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} i dati relativi alle diverse tipologie di domande presenti nel sistema. & Quizzipedia::\gl{Server}::\gl{Server}::\-Services::Questions & N.I.
	\\
	TIRoutingManager & Viene verificato che il sistema gestisca in modo adeguato la comunicazione con il \gl{client} attraverso le API e il socket per indirizzare correttamente le richieste dell’utente ai corretti services. & Quizzipedia::\gl{Server}::RoutingManager & N.I.	
	\\
	\rowcolor{white}  
...
\\ Attraverso l'incremento dei documenti con ad esempio l'individuazione di nuove metriche che possano migliorare la qualità dei processi e grazie all'esperienza accumulata dai componenti del \gl{team}, i processi e la loro organizzazione sono migliorati portando a raggiungere il livello 3 del lo \gl{CMM}.
...
\\ Per il processo di verificazione il \gl{team} ha ritardato di 4 giorni rispetto a quanto pianificato, a causa del ritardo provocato dalla stesura della documentazione, come riportato sopra. 
...
\bold{Considerazioni finali:}
\\Per il processo di verificazione il \gl{team} ha usufruito delle risorse in proprio possesso senza alcuna necessità aggiuntiva. In tal modo i costi pianificati sono stati rispettati perfettamente.
...
\\ Il \gl{team} ha migliorato il proprio processo di verifica dei documenti attraverso l'utilizzo delle metriche definite in questo documento, consolidando quanto appreso nell'attività precedente e apportandone valore aggiunto, arrivando in tal modo a raggiungere il livello 3 del lo \gl{CMM}.
...
\end{tabella}
\bold{Considerazioni finali:} Dai risultati ottenuti è possibile stabilire che ogni documento presenta un buon grado di leggibilità. Infatti ogni valore rientra nella soglia di accettabilità imposta per tale metrica e addirittura un documento presenta un valore ottimale.
