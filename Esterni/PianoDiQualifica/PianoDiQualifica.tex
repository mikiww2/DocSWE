...
\begin{tabella}{!{\VRule}l!{\VRule}p{3cm}!{\VRule}p{6cm}!{\VRule}c!{\VRule}}
	...
	\\
	TIClient & Viene verificato che il sistema gestisca correttamente la componente Model relativa al \gl{design pattern} \gl{MVC}. Più precisamente, viene verificato che la memorizzazione della struttura dei dati avvenga correttamente. & Quizzipedia::Client::Client & N.I.
	\\
	...
	componente View relativa al pattern \gl{MVC}. Più precisamente, viene verificato che venga gestita correttamente la rappresentazione del prodotto al cliente. & Quizzipedia::Client::ViewClient & N.I.
	\\
	...
	\\
	TICtrlUsers & Viene verificato che sia gestita correttamente dal sistema la comunicazione tra la View, il Model e il \gl{server} per quanto riguarda le informazioni dell’utente e la loro gestione. & Quizzipedia::Client::ControllerClient::\-CtrlUsers & N.I.
	\\
	TIServer & Viene verificato che il sistema
	gestisca correttamente l’interazione tra le componenti del \gl{back-end}. & Quizzipedia::Server & N.I.
	\\
	TIControllerServer & Viene verificato che il sistema gestisca in modo corretto le richieste eseguite lato \gl{client}. & Quizzipedia::Server::ControllerServer & N.I.
	\\
	TIAuthenticationManager & Viene verificato che il sistema gestisca correttamente i servizi di autenticazioni richiesti dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-AuthenticationManager & N.I.
	\\
	TIClassManager & Viene verificato che il sistema gestisca correttamente le richieste di gestione delle classi provenienti dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-ClassManager & N.I.
	\\
	TICompanyManager & Viene verificato che il sistema gestisca appropriatamente le richieste di gestione degli enti ricevute dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-CompanyManager & N.I.
	\\
	TIProfileManager & Viene verificato che il sistema gestisca appropriatamente le richieste del \gl{client} per la gestione delle informazioni riguardanti i profili degli utenti autenticati. & Quizzipedia::Server::ControllerServer::\-ProfileManager & N.I.
	\\
	TIQuestionsManager & Viene verificato che il sistema gestisca in modo corretto la gestione delle domande secondo le richieste ricevute dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-QuestionsManager & N.I.
	\\
	TIQuizManager & Viene verificato che il sistema gestisca in modo corretto la gestione dei quiz secondo le richieste ricevute dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-QuizManager & N.I.
	\\
	TIQMLAgent & gestisca in modo corretto la traduzione delle informazioni inviate dal \gl{client}, da linguaggio \gl{QML} ad un formato corretto per il sistema. Deve essere verificato che avvenga anche il contrario. & Quizzipedia::Server::ControllerServer::\-QuizManager::QMLAgent & N.I.
	\\
	TIRequestsManager & Viene verificato che il sistema gestisca in modo appropriato le richieste degli utenti (di ruolo o di entrata in una classe) inviate dal \gl{client}. & Quizzipedia::Server::ControllerServer::\-RequestsManager & N.I.
	\\
	TISearchManager & Viene verificato che il sistema gestisca correttamente le informazioni inviate dal \gl{client} qualora un utente abbia eseguito una ricerca lato \gl{client}. Deve essere verificata anche la comunicazione della risposta al \gl{client}. &  Quizzipedia::Server::ControllerServer::\-SearchManager & N.I.
	\\
	TIStatisticsManager & Viene verificato che il sistema gestisca correttamente le richieste delle statistiche ricevute dal \gl{client}, riguardanti quiz, domande e utenti. &  Quizzipedia::Server::ControllerServer::\-StatisticsManager & N.I.
	\\
	TITopicManager & Viene verificato che il sistema gestisca correttamente le richieste di creazione e di eliminazione di un argomento da parte del \gl{client}. & Quizzipedia::Server::ControllerServer::\-TopicManager & N.I.
	\\
	TIServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} tutti i dati ottenuti dal \gl{client}.  &  Quizzipedia::Server::Server & N.I.
	\\
	TIServicesServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} tutti i dati riguardanti le domande e i quiz. &  Quizzipedia::Server::Server::\-Services & N.I.
	\\
	TIQuestionsServer & Viene verificato che il sistema gestisca correttamente lato \gl{server} i dati relativi alle diverse tipologie di domande presenti nel sistema. & Quizzipedia::Server::Server::\-Services::Questions & N.I.
	\\
	TIRoutingManager & Viene verificato che il sistema gestisca in modo adeguato la comunicazione con il \gl{client} attraverso le API e il socket per indirizzare correttamente le richieste dell’utente ai corretti services. & Quizzipedia::Server::RoutingManager & N.I.	
	\\
	\rowcolor{white}  
...
\begin{tabella}{l!{\VRule}>{\centering\arraybackslash}p{6 cm}!{\VRule}>{\centering\arraybackslash}p{2 cm}!{\VRule}>{\centering\arraybackslash}p{2 cm}}
	...
	\multirow{-2}{*}{\cellcolor{D}Processo di Verifica} & Capability Maturity Model & ???? \\
	\hline
