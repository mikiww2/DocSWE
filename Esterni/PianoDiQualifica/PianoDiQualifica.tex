...
\begin{diario}
	\recap{Approvazione del documento}{Filippo Tesser}{Responsabile}{?????}{3.0}
	\recap{Correzione errori individuati}{Michela De Bortoli}{Analista}{??????}{2.10}
	\recap{Verifica dell'intero documento}{Rudy Berton}{Verificatore}{??????}{2.9}
	\recap{Stesura appendice D}{Giacomo Beltrame}{Analista}{??????}{2.8}
	\recap{Verifica appendici A e B}{Giacomo Beltrame}{Verificatore}{??????}{2.7}
	\recap{Stesura test di integrazione}{Rudy Berton}{Amministratore}{?????}{2.6}
	\recap{Stesura test di sistema}{Vassilikì Menarin}{Progettista}{????}{2.5}
	\recap{Modifica della sezione A.3.0.3 dell'appendice}{Filippo Tesser}{Analista}{?????}{2.4}
	\recap{Incremento test di accettazione}{Michela De Bortoli}{Progettista}{???????}{2.3}
	\recap{Inizio stesura specifica dei test (appendice B)}{Michela De Bortoli}{Progettista}{??????}{2.2}
	\recap{Incremento dell'appendice A}{Filippo Tesser}{Analista}{???????}{2.1}
...
%La strategia che verrà seguita per la verifica dei moduli sarà bottom-up, ovvero  integrare prima le parti con minore dipendenza funzionale e maggiore funzionalità
%che corrispondono alle componenti per requisiti obbligatori permettendo, in questo modo, di avere una versione
%funzionante delle parti obbligatorie dell’applicazione il prima possibile.
% 
%Questi test sono stati pensati seguendo una strategia bottom-up, ossia basata sulla logica di rea-
%lizzare il prodotto G finale a partire dalle singole componenti necessarie per realizzare le funzioni
%richieste.
%
%
%
%Verrà utilizzata una strategia di integrazione incrementale per poter sviluppare e
%verificare più componenti in parallelo. Questo metodo permette di dare priorità ai test
%relativi alle componenti che vengono ritenute più importanti. In questo modo sarà possibile
%partire dalle componenti che soddisfano i requisiti obbligatori fino ad integrarli con le
%componenti che soddisfano i requisiti opzionali.
%L’ordine di integrazione delle parti è bottom-up: prima di tutto verranno inserite le
%componenti con minori dipendenze funzionali e maggiori funzionalità, che corrispondono
%cioè ai requisiti obbligatori. Questi componenti saranno perciò testati molte più volte,
%per ridurre la possibilità che il prodotto finale contenga difetti, e si otterrà una versione
%funzionante dell’applicazione nel minor tempo possibile. In seguito si risalirà man mano
%l’albero delle dipendenze, fino all’inserimento delle componenti più di alto livello.
%Tale metodo è molto più oneroso rispetto ad altri in quanto richiede che venga
%generato del codice di supporto, sotto forma di driver e stub, che simuli le componenti
