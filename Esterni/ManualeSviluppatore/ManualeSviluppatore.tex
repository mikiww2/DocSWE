...
%	\listoftables
	...
	\begin{itemize}
		...
		\item inserire il caso che riconosca tale stringa nella funzione \texttt{generate} e \texttt{parse};
		...
		\item inserire la chiamata delle due funzioni nel caso definito al punto 2.
	\end{itemize}
	...
	\newpage
	\section{Gestione dei responsabili} %DA FINIRE
	Una volta installato il prodotto viene creato di default un account SuperAdmin che permette di creare enti e l'account responsabile associato. Per essere reperibili da chiunque voglia diventare responsabile di un ente è necessario quindi modificare il form "Contattaci" inserendo la proprio mail come referente. Una volta che un SuperAdmin riceve per mail una richiesta di aspiranti responsabili, può loggare con le credenziali    , troverà un form speciale che permette l'inserimento nel database di account responsabili ed enti.
