...
\newline
invia il quiz completo al \gl{server} perché lo salvi nel \gl{database}
\begin{itemize}
...
\newline
invia al \gl{server} le informazioni necessarie per compiere la ricerca nel \gl{database}
\begin{itemize}
...
\item \method{loadTopics () : void}
\newline
effettua una richiesta al \gl{server} che mi ritorna un array di topics che verrà poi assegnato allo \$scope topicsList
\item \method{removeTopic (indexOfTopic : int) : void}
\newline
rimuove l'argomento specificato in locale e invia una richiesta al \gl{server}
\begin{itemize}
\item \attribute{indexOfTopic : int}
\newline
...
\begin{figure}[H]
\centering
\noindent\makebox[\textwidth]{\includegraphics[scale=0.75]{Img/quizzipedia-client-viewmodelclient-ctrlservices-ctrlquestions-mymutiplechoiceq.pdf}}
...
\newline
richiede al \gl{server} le statistiche relative al quiz selezionato e aggiorna lo \$scope
\end{itemize}
\end{itemize}
...
\item \attribute{result : StudentsStatistcsQuiz}
\newline
...
\newline
invia al \gl{server} le informazioni necessarie per portare a termine la registrazione dell'utente
\begin{itemize}
\item \attribute{user : \$scope}
\newline
utilizza l'oggetto MyUser definito nello \$scope e invia al \gl{server} le informazioni necessarie per memorizzare l'utente
\end{itemize}
\end{itemize}
\end{itemize}
...
\item \method{loadUser () : void}
\newline
si occupa di comunicare col \gl{server} per poter avere le informazioni dell'utente
\end{itemize}
\end{itemize}
...
\item \bold{Metodi}
\begin{itemize}
\item \method{login (email : String, password : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{recoverPassword () : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{adder (class : \gl{JSON}, classname : String) : void}
\newline
permette la creazione di una nuova classe (che sarà associata ad un ente)
\begin{itemize}
\item \attribute{class : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{getStudents (classname : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{getTeachers (classname : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{fetchData (email : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{fetchQuiz (email : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{createQuestion (question : \gl{JSON}) : void}
\newline
permette il salvataggio di una nuova domanda
\begin{itemize}
\item \attribute{question : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{updateQuestion (question : \gl{JSON}) : void}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{updateStats (question : String, statistics : \gl{JSON}) : void}
\newline
...
\item \attribute{statistics : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{createQuiz (quiz : \gl{JSON}) : void}
\newline
permette la creazione di un nuovo quiz
\begin{itemize}
\item \attribute{quiz : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{fetchQuiz (quiz : String) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{updateQuiz (quiz : \gl{JSON}) : void}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{updateResults (quiz : String, results : \gl{JSON}) : void}
\newline
...
\item \attribute{results : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{updateStats (quiz : String, statistics : \gl{JSON}) : void}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{generate (question : \gl{JSON}) : String}
\newline
traduzione in formato \gl{QML} di un quiz per procedere con il salvataggio nel \gl{database}
\begin{itemize}
\item \attribute{question : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{parse (question : String) : \gl{JSON}}
\newline
traduzione del formato \gl{QML} in formato \gl{JSON} per la visualizzazione lato \gl{client}
\begin{itemize}
\item \attribute{question : String}
\newline
la domanda da tradurre
...
\item \bold{Metodi}
\begin{itemize}
\item \method{fetchRequests () : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{searchQuestions (parameters : \gl{JSON}) : \gl{JSON}}
\newline
ritorna le domande che corrispondono ai parametri di ricerca impostati dall'utente
\begin{itemize}
\item \attribute{parameters : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{searchQuiz (parameters : \gl{JSON}) : \gl{JSON}}
\newline
...
\item \bold{Metodi}
\begin{itemize}
\item \method{fetchStats () : \gl{JSON}}
\newline
ritorna le statistiche riferite ad una domanda
\end{itemize}
\end{itemize}
...
\item \bold{Metodi}
\begin{itemize}
\item \method{<<abstract>> forwardTo (bundle : \gl{JSON}) : void}
\newline
metodo astratto usato per il reindirizzamento
\begin{itemize}
\item \attribute{bundle : \gl{JSON}}
\newline
contenitore dei dati provenienti da chiamate REST del \gl{client}
\end{itemize}
\item \method{<<abstract>> forwardTo (bundle : \gl{JSON}) : \gl{JSON}}
\newline
metodo astratto usato per il reindirizzamento
\begin{itemize}
\item \attribute{bundle : \gl{JSON}}
...
contenitore dei dati provenienti da chiamate REST del \gl{client}
\end{itemize}
\item \method{<<abstract>> forwardTo () : \gl{JSON}}
\newline
metodo astratto usato per il reindirizzamento
...
\item \method{forwardTo (bundle : \gl{JSON}) : void}
\newline
metodo usato per il reindirizzamento alla classe AuthenticationManager
\begin{itemize}
\item \attribute{bundle : \gl{JSON}}
...
\item \bold{Metodi}
\begin{itemize}
\item \method{forwardTo (bundle : \gl{JSON}) : \gl{JSON}}
\newline
